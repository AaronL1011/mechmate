	...
	>
		<div class="mb-2 flex items-center gap-2">
			<span class="text-sm font-medium text-gray-900 capitalize dark:text-white">
				{action.type}
				{action.entity.replace('_', ' ')}
			</span>
		</div>
		<p class="mb-3 text-sm text-gray-700 dark:text-gray-300">
			{action.confirmation_message ||
				`I'll ${action.type} this ${action.entity.replace('_', ' ')} with the details below. Feel free to edit any values or add additional instructions.`}
		</p>

		{#if action.data && !isLoading}
			<div class="mb-3 space-y-2">
				{#each formatData(action.data) as item (item.key)}
					<div class="flex flex-col gap-1">
						...
						{#if item.editable}
							...
							{#if inputType === 'select'}
								{#if item.fieldKey === 'equipment_type_id'}
									<select
										...
										onchange={(e) =>
											updateField(item.fieldKey, parseInt((e.target as HTMLSelectElement).value))}
										class="rounded border border-gray-200 bg-white px-2 py-1 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
									>
							...
							{:else if inputType === 'parts_array'}
								...
								<div class="space-y-2">
									{#each partsArray as part, index (part)}
										<div class="flex items-center gap-2">
											<input
												...
												class="flex-1 rounded border border-gray-200 bg-white px-2 py-1 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
											/>
											<button
												type="button"
							...
							{:else if inputType === 'textarea'}
								<textarea
									...
									rows="3"
									class="resize-none rounded border border-gray-200 bg-white px-2 py-1 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
								></textarea>
						...
						{:else}
							<span
								class="rounded bg-gray-100 px-2 py-1 text-sm text-gray-900 dark:bg-gray-700 dark:text-white"
							>
								{item.value}
							</span>
						{/if}
