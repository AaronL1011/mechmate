			...
			<div class="flex flex-col gap-3">
				<div class="flex min-w-0 items-start justify-between gap-3">
					...
					<div class="mb-1.5 flex flex-wrap items-center gap-2">
						<span
							class="inline-flex shrink-0 rounded-md bg-gray-200/80 px-2 py-0.5 text-xs font-medium uppercase tracking-wide text-gray-600 dark:bg-gray-600/60 dark:text-gray-300"
						>
					...
					{#if equipment.make || equipment.model || equipment.year}
						...
						>
							{[
								equipment.make,
								equipment.model,
								equipment.year ? String(equipment.year) : null
							]
								.filter(Boolean)
								.join(' · ')}
						</p>
				...
				{#if equipment.next_due_summary}
					<div class="w-full">
						<span
							class="inline-flex items-center gap-1 rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-xs font-medium text-amber-800 dark:border-amber-800/50 dark:bg-amber-900/30 dark:text-amber-200 w-auto"
						>
							<svg
			...
			>
				<div class="flex items-baseline gap-1.5">
					<span class="text-xs font-medium uppercase tracking-wider text-gray-400 dark:text-gray-500"
						>Usage</span
					>
					<span class="text-sm font-medium tabular-nums text-gray-700 dark:text-gray-200">
						{equipment.current_usage_value}
						<span class="font-normal text-gray-500 dark:text-gray-400"
							>{equipment.usage_unit}</span
						>
					</span>
				</div>
				<div class="flex items-baseline gap-1.5">
