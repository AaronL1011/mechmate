...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    // Basic validation
    if (body.usage_interval !== null && body.usage_interval !== undefined && body.usage_interval <= 0) {
      return json({ error: 'Usage interval must be positive' }, { status: 400 });
    }
    
    if (body.time_interval_days !== null && body.time_interval_days !== undefined && body.time_interval_days <= 0) {
